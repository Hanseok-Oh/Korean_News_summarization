...
class crawling:
    def __init__(self,max_page,query,s_date, e_date,result_path = os.getcwd().replace("\\","/") + '/data'):
    ...
    def crawler(self):
        ...
        while page < maxpage_t:
            ...
            url = "https://search.naver.com/search.naver?where=news&query=" + self.query + "&sort=0&ds=" + s_date + "&de=" + e_date + "&nso=so%3Adr%2Cp%3Afrom" + s_from + "to" + e_to + "%2Ca%3A&start=" + str(
                page)
    ...
    def excel_make(self,query,date):
        data = pd.read_csv(self.result_path + '/{}_contents_text.txt'.format(query), sep='\t', header=None,error_bad_lines=False)
